...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  if (!hasPremiumPlan) {
    throw new Error(
      "Precisa de uma subscrição Premium para poder gerar relatórios IA",
    );
  }
  ...
  const content = `Gera um relatório com insights sobre as minhas finanças, com dicas e orientações de como melhorar a minha vida financeira. As transações estão divididas por ponto e vírgula. A estrutura de cada uma é {DATA}-{TIPO}-{VALOR}-{CATEGORIA}. Aqui estão elas: 
  ${transactions
    .map(
      (transaction) =>
        `${transaction.date.toLocaleDateString("pt-PT")}-€${transaction.amount}-${transaction.type}-${transaction.category}`,
    )
    .join(";")}`;
    ...
    messages: [
      {
        role: "system",
        content:
          "És um especialista em gestão e organização de finanças pessoais. Ajudas as pessoas a organizarem melhor as suas finanças.",
      },
      {
        role: "user",
        content,
